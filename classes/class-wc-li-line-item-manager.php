...
class WC_LI_Line_Item_Manager {
    ...
    /**
     * WC_LI_Line_Item_Manager constructor.
     *
    ...
    public function build_products($order) {
        ...
        if (count($items) > 0) {
            ...
            } elseif (($order->billing_country === 'IL') ||
                    //($order->billing_country === 'GB') ||
                    ($order->billing_country === 'Israel') //||
            //($order->billing_country === 'United Kingdom (UK)')
            ) {
                $sales_account = $this->settings->get_option('sales_account_IL');
            } else {
            ...
            foreach ($items as $item) {
                ...
                $line_item = new WC_LI_Line_Item($this->settings);
...
//				if ( true === $send_inventory && $product->is_on_sale() ) {} // Set the unit price if we send inventory and the product is on sale
     ...
     *
     * @param WC_Order $order
     * @param WC_LI_Line_Item[] $line_items
     *
     * @return WC_LI_Line_Item
     */
    public function build_correction($order, $line_items) {
        ...
        if ($order_total !== $line_total) {
            ...
            /*
              ...
              $correction_line = new WC_LI_Line_Item( $this->settings );
